...
#define _CRT_SECURE_NO_WARNINGS // Suppress warnings for unsafe functions in MSVC
...
#define CTRL_KEY(k) ((k) & 0x1f) // Macro that returns the ASCII equivalent of CTRL + letter
...
int editorReadKey() {
  int c = _getch();
  if (c == 224 || c == 0) { // Arrow keys or function keys
    int seq = _getch();
    switch (seq) {
...
int getCursorPosition(int *rows, int *cols) {
  ...
  *cols = csbi.dwCursorPosition.X + 1; //zero based
